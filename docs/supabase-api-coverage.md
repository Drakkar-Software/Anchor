...
**The `queryFn` escape hatch is narrower than it looks.** The callback's result is coerced to `{data: Row[], count, error}` via `r.data ?? []`, so `.single()`, `.maybeSingle()`, `.csv()` and `.geojson()` are callable through it but return shapes the store path cannot consume. A per-call `queryFn` is also outside per-query scoping, since 2.1.0: a function cannot be keyed by value, so the fetch registers no entry, gets a private de-duplication key, and reads back through the whole-store projection (`query/queryKey.ts`'s `isKeyable`). A store-level `defaultQueryFn` does not have that cost — it is the same function for every query, so the rest of the options still tell them apart.

Until 2.1.0 the builder it received had only `.select()` applied: `executeQuery` returned from the `queryFn` branch *before* the filters/pagination block, so a fetch carrying both a `queryFn` and filters ran unfiltered while `fetch` went on computing those filters and keying the query on them. Filters, `select`, `limit` and `offset` now all reach it. `sort` does not, and that asymmetry is deliberate: `order` is the one PostgREST parameter that accumulates rather than overwrites, so pre-applying it would demote the callback's own `.order()` to a tiebreaker behind `defaultSort`.
...
**`upsert` takes an options object; the other four mutations still do not.** `store.upsert(row, { onConflict })` reaches `.upsert()` in both `createTableStore.ts` and the replay in `mutationPipeline.ts`, and rides on `QueuedMutation.upsertOptions` so a drain writes the row the live call did. `insert`/`insertMany`/`update`/`remove`/`removeWhere` pass none — verified across `createTableStore.ts`, `mutationPipeline.ts`, `batchOperations.ts`. Unreachable everywhere, upsert included: `defaultToNull`, `count`, `returning: 'minimal'`.
...
**Telling a refusal from a dead network is a `status`, not a `catch`.** postgrest-js does not throw a failed fetch: `PostgrestBuilder.then` catches it into the ordinary `{ data, error }` pair (`dist/index.cjs:328` on 2.108.2, `:394` on 2.112.3) with `status: 0` and `code: ""`. A refusal arrives through the same slot carrying Postgres' `code` and a real HTTP status. Only the second may be thrown at the caller; only the first may be queued.
...
No third options argument is ever passed to `.rpc()` (`rpc/rpcAction.ts`, `query/aggregation.ts:35`) → `head`, `get`, `count` are all unreachable. `.rpc()` is only ever called on the root client — **RPCs in non-public schemas are unreachable even from a schema-scoped store**.

2.1.0 added the `Functions` extractors `types.ts` never had (`FunctionNames`, `RpcArgs`, `RpcReturns`) and `createSchemaRpc<DB>(supabase)`, which reads the name, the argument object and the return type off the generated `Database`. `callRpc` itself keeps its signature and its `data as T`: its first type argument is the return type, so making it generic over `DB` would bind `Database` at every existing call site that means something else. `useRpc` is likewise untouched and still takes an explicit `T`. (`Views` had no extractor until the same release — now `ViewNames`/`ViewRow`, behind `createSupabaseStores`' `views:` option.)
