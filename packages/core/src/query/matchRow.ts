...
function matchOne(row: Record<string, unknown>, filter: FilterDescriptor<any>): boolean {
  ...
  switch (op) {
    ...
    case "contains": {
      ...
      if (!Array.isArray(actual) || !Array.isArray(value)) {return true}

      {
        const actualSet = new Set(actual)

        return value.every((v) => actualSet.has(v))
      }
    }
    case "containedBy": {
      if (!Array.isArray(actual) || !Array.isArray(value)) {return true}

      {
        ...
        return actual.every((v) => valueSet.has(v))
      }
    }
    case "overlaps": {
      if (!Array.isArray(actual) || !Array.isArray(value)) {return true}

      {
        ...
        return actual.some((v) => valueSet.has(v))
      }
    }
