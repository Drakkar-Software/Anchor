...
import { type FilterDescriptor,randomId,type TableStore, type TrackedRow  } from "../types.js"
...
>(
  ...
  try {
    ...
    store.setState((prev: any) => {
      ...
      return { ...prev, error: error instanceof Error ? error : new Error(String(error)), order, records }
    })
...
>(
  ...
  for (const [id, record] of state.records) {
    const matches = filters.every((f) => {
      ...
      switch (f.op) {
        ...
        default: { return true
        } // Server will do the actual filtering
      }
    })
  ...
  try {
    ...
    store.setState((prev: any) => {
      ...
      return { ...prev, error: error instanceof Error ? error : new Error(String(error)), records }
    })
