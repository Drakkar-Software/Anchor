...
describe("mockSupabase filter operators", () => {
  function client() {
    return createMockSupabase({
      posts: [
        { id: 1, title: "hello world", tags: ["red", "blue"], meta: { pinned: true }, views: 10 },
        { id: 2, title: "goodbye", tags: ["blue"], meta: { pinned: false }, views: 20 },
        { id: 3, title: "hello again", tags: ["green"], meta: { pinned: true }, views: 30 },
      ],
    })
  }

  const ids = (data: any[]) => data.map((r: any) => r.id).sort()
  ...
  it("throws on an operator it has not implemented instead of matching everything", async () => {
    ...
    await expect(builder).rejects.toThrow(/unimplemented filter operator/)
  })
...
describe("mockSupabase update and delete honour every filter", () => {
  function client() {
    return createMockSupabase({
      posts: [
        { id: 1, views: 10, archived: false },
        { id: 2, views: 20, archived: false },
        { id: 3, views: 30, archived: false },
      ],
    })
  }
...
describe("mockSupabase error injection", () => {
  it("fails a select with the injected error and status", async () => {
    const supabase = createMockSupabase({ posts: [{ id: 1 }] })
    supabase._setError("posts", "select", { message: "boom", code: "42501" }, { status: 403 })
  ...
  it("injects status 0, which is what isTransportError requires", async () => {
    ...
    supabase._setError("posts", "insert", { message: "fetch failed", code: "" }, { status: 0 })
...
describe("mockSupabase write responses", () => {
  ...
  it("returns the count without a body for head: true", async () => {
    ...
    const { data, count } = await supabase.from("posts").select("*", { count: "exact", head: true })
    expect(data).toBeNull()
    expect(count).toBe(2)
  })
})
...
describe("mockSupabase realtime channels", () => {
  ...
  it("records what was broadcast, so a fire-and-forget send is observable", async () => {
    ...
    await channel.send({ type: "broadcast", event: "sync", payload: { a: 1 } })

    expect(channel._sent).toEqual([{ type: "broadcast", event: "sync", payload: { a: 1 } }])
  })
