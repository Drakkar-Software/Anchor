    ...
    storage: {
      from: vi.fn().mockReturnValue({
        ...
        list: vi.fn().mockResolvedValue(
          overrides.list ?? { data: [{ id: "1", metadata: {}, name: "file.png" }], error: null },
        ),
...
describe("storage error routing", () => {
  ...
  it("keeps the code on an upload failure", async () => {
    const supabase = mockStorage({
      upload: { data: null, error: { code: "409", message: "The resource already exists" } },
    })
  ...
  it("keeps the code on a download failure", async () => {
    const supabase = mockStorage({
      download: { data: null, error: { code: "404", message: "Object not found" } },
    })
